...
const dummyUser = {
  ...
  about:
    "Passionate about algorithms, web development, and open source. Always learning.",
  skills: [
    "JavaScript",
    "Python",
    "React",
...
function StatCard({
  ...
  return (
    <div
      className={`group relative bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-2xl hover:shadow-3xl transition-all duration-500 hover:-translate-y-2 hover:border-gray-700 ${
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl p-8 mb-8">
      ...
      <div className="flex flex-wrap gap-4">
        {badges.map((badge, index) => (
          <div
            key={badge.id}
            className={`group flex items-center gap-3 bg-gradient-to-r from-gray-800 to-gray-700 border border-gray-600 text-white px-6 py-3 rounded-full hover:from-purple-700 hover:to-purple-600 hover:border-purple-500 transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-2xl transform ${
...
function RecentActivity({
  ...
  activity: UserType["recentActivity"];
}) {
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl p-8 mb-8">
      <h3 className="text-2xl font-bold mb-6 text-white flex items-center gap-3">
      ...
      <div className="space-y-4">
        {activity.map((item, index) => (
          <div
            key={item.id}
            className={`group flex items-center justify-between p-4 rounded-xl hover:bg-gray-800/50 transition-all duration-300 border border-transparent hover:border-gray-700 ${
              index === 0
                ? "bg-gradient-to-r from-gray-800/30 to-purple-900/20 border-gray-700"
                : ""
            }`}
          >
            <div className="flex items-center gap-4">
              <div
                ...
                } group-hover:scale-125 transition-transform duration-300 shadow-lg`}
              ></div>
...
function SolvedProblemsTable({
  problems,
}: {
  problems: UserType["solvedProblems"];
}) {
  return (
        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b-2 border-gray-700">
              <th className="py-4 px-6 text-left font-semibold text-gray-300 uppercase tracking-wider">
                Problem
              </th>
              <th className="py-4 px-6 text-left font-semibold text-gray-300 uppercase tracking-wider">
                Difficulty
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl p-8 mb-8">
      ...
      <div className="flex flex-wrap gap-3">
        {skills.map((skill, index) => (
          <span
            key={index}
            className={`px-4 py-2 bg-gray-800 border border-gray-700 text-gray-200 rounded-full text-sm font-semibold hover:bg-gray-700 hover:border-purple-600 hover:text-purple-300 transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-2xl cursor-default ${
              index % 3 === 0
                ? "hover:rotate-1"
                : index % 3 === 1
                ? "hover:-rotate-1"
  ...
  return (
    <footer className="mt-16 py-8 text-center text-gray-500 border-t border-gray-800">
      ...
      <p className="text-sm text-gray-400">
        © 2024 CodeCraft. Inspired by LeetCode. All rights reserved.
      </p>
    </footer>
  );
}
